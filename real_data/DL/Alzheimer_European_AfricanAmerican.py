...
def run_cv(seed):
    ...
    para_mix = {'batch_size': 50, 'momentum': 0.9, 'n_epochs': 100,
                'learning_rate': 0.15, 'lr_decay': 0.03, 'dropout': 0.5,
                'L1_reg': 0.0015, 'L2_reg': 0.0015, 'hidden_layers_sizes': [200]}  # 0.71, 0.70, 0.52
    para_ind_EA = {'batch_size': 50, 'momentum': 0.9, 'n_epochs': 100,
                'learning_rate': 0.15, 'lr_decay': 0.03, 'dropout': 0.5,
                'L1_reg': 0.0015, 'L2_reg': 0.0015, 'hidden_layers_sizes': [200]}
    para_ind_AA = {'batch_size': 50, 'momentum': 0.9, 'n_epochs': epoch,
    ...
    para_super = {'batch_size': 32, 'momentum': 0.9, 'n_epochs': 20, 'tune_batch': 20,
                  'learning_rate': 0.01, 'lr_decay': 0.0, 'dropout': 0.5, 'tune_lr': 0.01, 'tune_epochs': 200,
                  'L1_reg': 0., 'L2_reg': 0.0001, 'hidden_layers_sizes': [50]}

    Aggr, EA, Minor = get_data("data/AD/EA_AA1.csv", seed=seed)
...
def main():
    ...
    for seed in range(20):
        row = run_cv(seed)
        res.append(row)
    df_res = pd.DataFrame(res, columns=['Mix0_DL', 'Mix1_DL', 'Mix2_DL', 'Ind1_DL', 'Ind2_DL', 'NT_DL', 'TL_DL'])
    print(df_res)
