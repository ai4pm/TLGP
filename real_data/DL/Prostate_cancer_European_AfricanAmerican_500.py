...
def mixture_learning(Aggr, minor='AA', **kwds):
    ...
    Map = {}
    Map["Mix0_DL"], Map["Mix1_DL"], Map["Mix2_DL"] = A_AUC, EA_AUC, AA_AUC
    return Map
...
def run_cv(seed):
    ...
    for seed in range(20):
        ...
        Map['Ind1_DL'] = independent_learning(EA, **para_ind_EA)
        Map['Ind2_DL'] = independent_learning(Minor, **para_ind_AA)
        Map['NT_DL'] = naive_transfer(EA, Minor, **para_naive)
        _, EA_s, Minor_t = prepare_data(train, test, k, seed)
        Map['TL_DL'] = super_transfer(EA_s, Minor_t, **para_super)
