...
def super_transfer(EA, Minor, **kwds):
    ...
    model.fit(EA_train_data[0], EA_train_data[1], validation_data=EA_val_data, batch_size=batch_size, epochs=n_epochs,
              callbacks=[es], verbose=0)
    K.set_value(model.optimizer.lr, tune_lr) # 0.015
    ...
    return np.round(A_AUC, decimals=3)
...
def get_data(file, seed=0):
    ...
    EAA_X_train, EAA_Y_train = X_train[R_train == 'AMR'], Y_train[R_train == 'AMR']
    EA_X_val, EA_Y_val = X_val[R_val == 'EUR'], Y_val[R_val == 'EUR']
    EAA_X_val, EAA_Y_val = X_val[R_val == 'AMR'], Y_val[R_val == 'AMR']
    EA_X_test, EA_Y_test = X_test[R_test == 'EUR'], Y_test[R_test == 'EUR']
    EAA_X_test, EAA_Y_test = X_test[R_test == 'AMR'], Y_test[R_test == 'AMR']
...
def run_cv(seed):
    ...
    Aggr, EA, Minor = get_data("../SD5.mat", seed=seed)
